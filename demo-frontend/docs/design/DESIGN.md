...
## 12. Лендинг (по запросу пользователя, 23 сентября 2026)

Маршрут `/` объясняет ценность Delphi и ведёт в готовую проверку; история доступна на `/history`. Лендинг использует существующие Button, StatusChip, Inter и доменные цвета. Локальные стили — `src/styles/landing.css` и `src/styles/transfer-explainer.css`; они не меняют рабочие экраны. Цвет текста лендинга `#13243b`, фон `#fbfcfe`, вторичный текст `#627083`, тёмная секция `#14283e`; основной CTA — существующий primary.

По явному запросу на лендинг с анимациями для этой страницы действует отдельное правило вместо ограничений §7: управляемое переключение трёх шагов (260 мс ease-out), однократное появление разделов (550 мс ease-out) и плавная навигация по якорям. Без автоматического перелистывания. prefers-reduced-motion оставляет статичное содержимое.

Главный пример — синтетический transfer: исполнитель меняется с Отдела аудита на Службу рисков, цитаты совпадают с fixture. Лендинг не запускает модель, не имитирует сохранение решений и явно описывает ограничения демо.
