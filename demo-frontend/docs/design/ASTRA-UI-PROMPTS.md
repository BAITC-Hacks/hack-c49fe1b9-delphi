...
## Правки по макету Figma — после интеграции API (с 16:30)

Правила для Codex теперь в [`demo-frontend/AGENTS.md`](../../AGENTS.md): Codex читает его сам, когда работает в `demo-frontend/`, и он перекрывает корневой `AGENTS.md`. Раздел «Вставить в `AGENTS.md`» ниже устарел.

Стартовый промпт — один на экран, подставить ссылку и экран:

```
Репозиторий hack-c49fe1b9-delphi, новая ветка от актуальной nurdaulet-api-integration (PR #2). Работаешь только в demo-frontend/.
Сначала прочитай demo-frontend/AGENTS.md и всё, что он велит прочитать. До этого ничего не меняй.

Задача: перенести в код макет из Figma: <ссылка на фрейм>. Экран: <например, «Функции и риски»>.

1. Открой текущий экран: npm run dev → http://localhost:5173/analyses/demo. Сравни с макетом и выпиши
   расхождения списком: что меняется и какую задачу пользователя из AGENTS.md («Design principle») это
   ускоряет. Пункт, который не ускоряет ни одну задачу, не делай — отметь «пропущено: только внешний вид».
2. Токены — сначала src/index.css и docs/design/DESIGN.md §2, потом компоненты. Бери существующие
   компоненты; новый — только если подходящего нет.
3. Не меняй src/lib/api.ts, src/lib/adapter.ts и типы Api*: данные экрана бери из текущей модели.
   Если макету нужны данные, которых нет в API, — не выдумывай, напиши об этом в ответе.
4. Проверь: npm run build; экран на 1280 и 375 px; пустое и ошибочное состояния; /analyses/demo.
5. Добавь строку в demo-frontend/docs/decisions.md, коммит «feat(ui): <экран> по макету Figma», PR в main.
   В ответе: что изменено, что пропущено и почему, скриншоты до/после.
```

## Вставить в `AGENTS.md` (раздел UI) до первого промпта (устарело, см. выше)
