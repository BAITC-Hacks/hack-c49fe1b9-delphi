...
Порядок жёсткий. Каждый промпт — отдельная задача Codex. Перед каждым: `git pull --rebase`. После каждого: `npm run build` в `demo-frontend/`, проверить в браузере, коммит только своих файлов (`demo-frontend/**`, `docs/design/**`, `docs/screenshots/**`, `README.md`).
