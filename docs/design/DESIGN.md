...
Это единственный источник правды по интерфейсу. Codex читает этот файл перед каждым UI-промптом из `docs/design/ASTRA-UI-PROMPTS.md`. Всё, чего здесь нет, — не делаем до 18:00.
...
| Слой | Выбор | Причина |
|---|---|---|
| Фреймворк | React 18 + TypeScript + Vite (папка `frontend/`). Если Осман выбрал Next.js — те же компоненты, App Router | Согласовано с планом команды |
...
## 2. Токены (`frontend/src/styles/tokens.css`)
...
| `conflict` | Возможный конфликт интересов | `AlertTriangle` | Исполнение и контроль в одних руках, и подобное. **К проверке.** |
...
Пример — это **не запуск анализа**, а мгновенный показ заранее посчитанного результата из `frontend/public/demo/result.json` с `ModeBadge = «Пример»`. Это страховка демо: интерфейс показывает результат даже при упавшем бэкенде.
...
## 6. Контракт данных для UI (`frontend/src/types.ts`)
...
Если есть готовый шаблон UX/UI: экспортировать 3–4 экрана в PNG в `docs/design/reference/`, положить рядом и указать Codex в промпте UI-1 «следуй композиции из `docs/design/reference/*.png`, токены и компоненты — из DESIGN.md». Codex читает изображения. Этого достаточно.
